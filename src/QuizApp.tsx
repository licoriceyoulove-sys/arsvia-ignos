...
//   // choice 用（「正解」と「不正解」を分離）
//   const [correctChoice, setCorrectChoice] = useState<string>("");
//   const [wrongChoices, setWrongChoices] = useState<string[]>(["", ""]); // 初期で2枠
...
//     if (type === "choice") {
//       const correctOk = correctChoice.trim().length > 0;
//       const wrongFilled = wrongChoices.map((s) => s.trim()).filter(Boolean);
//       // 「正解1つ＋不正解1つ以上」で最低2択を保証
//       return correctOk && wrongFilled.length >= 1;
//     } else {
//       return modelAnswer.trim().length > 0;
//     }
//   }, [question, tagsInput, type, correctChoice, wrongChoices, modelAnswer]);

//   // 不正解の追加・更新・削除
//   const addWrong = () => setWrongChoices((prev) => [...prev, ""]);
//   const updateWrong = (idx: number, val: string) =>
//     setWrongChoices((prev) => prev.map((c, i) => (i === idx ? val : c)));
//   const removeWrong = (idx: number) =>
//     setWrongChoices((prev) => prev.filter((_, i) => i !== idx));
...
//     if (type === "choice") {
//       const wrongFilled = wrongChoices.map((s) => s.trim()).filter(Boolean);
//       const choices = [correctChoice.trim(), ...wrongFilled];
...
//                 canPost ? "bg-black text-white" : "bg-gray-200 text-gray-400"
//               }`}
//             >
//               投稿
//             </button>
